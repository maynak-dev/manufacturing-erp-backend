import { PrismaClient, UserRole, OrderStatus, WorkOrderStatus, PurchaseOrderStatus, InvoiceStatus, PaymentStatus, DispatchStatus, InventoryType } from '@prisma/client';
...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.upsert({
      ...
      update: {},
      create: { email: 'admin@erp.com', password: passwordHash, name: 'Arjun Sharma', role: UserRole.ADMIN },
    }),
    prisma.user.upsert({
      where: { email: 'sales@erp.com' },
      update: {},
      create: { email: 'sales@erp.com', password: passwordHash, name: 'Priya Mehta', role: UserRole.SALES },
    }),
    prisma.user.upsert({
      where: { email: 'production@erp.com' },
      update: {},
      create: { email: 'production@erp.com', password: passwordHash, name: 'Ravi Kumar', role: UserRole.PRODUCTION },
    }),
    prisma.user.upsert({
      where: { email: 'procurement@erp.com' },
      update: {},
      create: { email: 'procurement@erp.com', password: passwordHash, name: 'Sunita Patel', role: UserRole.PROCUREMENT },
    }),
    prisma.user.upsert({
      where: { email: 'finance@erp.com' },
      update: {},
      create: { email: 'finance@erp.com', password: passwordHash, name: 'Deepak Nair', role: UserRole.FINANCE },
    }),
  ]);
  ...
  const workOrders = await Promise.all([
    ...
    prisma.workOrder.create({
      data: {
        ...
        notes: 'Awaiting raw material receipt PO-2025-001',
      },
    }),
  ]);
  ...
  // ─── NOTIFICATIONS ────────────────────────────────────────────────────────
  await Promise.all([
    prisma.notification.create({ data: { userId: users[0].id, title: 'New Order Received', message: 'Order ORD-2025-004 from Mahindra Logistics worth ₹3,82,500 has been placed.', isRead: false } }),
